...
@TRANSFORMS.register_module()
class RandomGTCrop(BaseTransform):
    """Random gt crop the image & bboxes & masks.
    ...
    def _crop_data(self, results: dict,
                   allow_negative_crop: bool) -> Union[dict, None]:
        """Function to randomly crop images, bounding boxes, masks, semantic
        segmentation maps.
        ...
        if num_gts > 0:
            # randomly select one gt
            if 'category_repeat' not in results:
                gt_index = random.randint(num_gts)
            else:
                category_repeat = results['category_repeat']
                gt_labels = results['gt_labels']
                label_repeat = []
                for label in gt_labels:
                    label_repeat.append(category_repeat[label + 1])
                sum_repeat = sum(label_repeat)
                label_prob = [repeat / sum_repeat for repeat in label_repeat]
                gt_index_list = [index for index in range(len(label_prob))]
                gt_index = np.random.choice(gt_index_list, p=label_prob)

            # randomly select other gts
            num_sel_gts = random.randint(1, self.random_gt)
            all_cx = (gt_bboxes[:, 2] + gt_bboxes[:, 0]) / 2
            all_cy = (gt_bboxes[:, 3] + gt_bboxes[:, 1]) / 2
            sel_cx = (gt_bboxes[gt_index, 2] + gt_bboxes[gt_index, 0]) / 2
            sel_cy = (gt_bboxes[gt_index, 3] + gt_bboxes[gt_index, 1]) / 2
            distances = (all_cx - sel_cx) ** 2 + (all_cy - sel_cy) ** 2
            sel_gt_inds = np.argsort(distances)[:num_sel_gts]
            x1, y1 = gt_bboxes[sel_gt_inds, :2].min(0)
            x2, y2 = gt_bboxes[sel_gt_inds, 2:].max(0)
            cx = (x1 + x2) / 2.
            cy = (y1 + y2) / 2.
            w = x2 - x1
            h = y2 - y1
            ...
            # randomly extend image crop
            random_extend = random.random()
            size = (1 + random_extend * self.extend_ratio) * side
            crop_size = [int(size), int(size)]
            min_offset_h = int(max(cy + h / 2. - size, 0))
            max_offset_h = int(max(cy - h / 2., 0))
            offset_h = np.random.randint(min_offset_h, max_offset_h + 1)
            min_offset_w = int(max(cx + w / 2. - size, 0))
            max_offset_w = int(max(cx - w / 2., 0))
            ...
            offset_h = 0
            offset_w = 0
            crop_y1, crop_y2 = offset_h, img_shape[0]
            crop_x1, crop_x2 = offset_w, img_shape[1]
...
@TRANSFORMS.register_module()
class LoadFeatures(BaseTransform):
    def __init__(self, cls_name_file, cls_emb_file, base_ind_file, novel_ind_file, num_texts=2) -> None:
        self.num_texts = num_texts
        
        lines = open(cls_name_file, 'r').readlines()
        self.cls_names = [line.strip() for line in lines]
        self.cls_embs = torch.load(cls_emb_file)
        
        line = open(base_ind_file, 'r').readlines()[0].strip()
        self.base_cls_inds = [int(string.strip()) for string in line.split(',')]
        line = open(novel_ind_file, 'r').readlines()[0].strip()
        self.novel_cls_inds = [int(string.strip()) for string in line.split(',')]

        print(f"############ all classes: {len(self.cls_names)}, base classes: {len(self.base_cls_inds)}, novel classes: {len(self.novel_cls_inds)} ##############")

    def transform(self, results: dict) -> dict:
        # prompts
        prompt_file = results['img_path'].replace('/images/', '/prompts/').replace('.jpg', '.txt')
        prompt = open(prompt_file, 'r').readlines()[0].strip()
        results['prompts'] = prompt

        # novel image flag
        novel_image_flag = torch.BoolTensor([False])
        cls_names = prompt.replace('a photograph of ', '').split(' and ')
        cls_inds = []
        for cls_name in cls_names:
            cls_ind = self.cls_names.index(cls_name)
            if cls_ind not in cls_inds:
                cls_inds.append(cls_ind)
                if cls_ind in self.novel_cls_inds:
                    novel_image_flag = torch.BoolTensor([True])
        results['novel_image_flag'] = novel_image_flag
        
        # modify label and record label mappings
        label_mappings = {}
        gt_bboxes_labels = results['gt_bboxes_labels'].copy()
        for grounding_label, dataset_label in enumerate(cls_inds):
            gt_bboxes_labels[results['gt_bboxes_labels'] == dataset_label] = grounding_label
            label_mappings[grounding_label] = dataset_label
        results['gt_bboxes_labels'] = gt_bboxes_labels
        results['label_mappings'] = label_mappings

        # text embeddings
        cls_embs = self.cls_embs[cls_inds]
        c1, c2 = cls_embs.shape  # for padding
        if c1 < self.num_texts:
            zero_embs = cls_embs.new_zeros(self.num_texts - c1, c2)
            cls_embs = torch.cat([cls_embs, zero_embs], 0)
        results['text_embeddings'] = cls_embs

        # diffusion features
        feat_file = results['img_path'].replace('/images/', '/features/').replace('.jpg', '.pt')
        diffusion_feats = torch.load(feat_file)
        results['diffusion_feats'] = diffusion_feats

        # base and novel class inds
        results['base_cls_inds'] = self.base_cls_inds
        results['novel_cls_inds'] = self.novel_cls_inds
