...
class CocoMetric(BaseMetric):
    ...
    def __init__(self,
                 ...
                 base_ind_file: Optional[str] = None,
                 novel_ind_file: Optional[str] = None) -> None:
        ...
        # fcj add
        if base_ind_file is not None and novel_ind_file is not None:
            base_inds = open(base_ind_file, 'r').readline().strip().split(', ')
            self.base_inds = [int(ind) for ind in base_inds]
            novel_inds = open(novel_ind_file, 'r').readline().strip().split(', ')
            self.novel_inds = [int(ind) for ind in novel_inds]
        else:
            self.base_inds = None
            self.novel_inds = None
        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal':
                ...
                if self.classwise:  # Compute per-category AP
                    ...
                    # fcj add
                    if self.base_inds is not None and self.novel_inds is not None:
                        # logger.info(results_per_category)
                        base_ap50_list = []
                        novel_ap50_list = []
                        for ind, (cls_name, map, ap50, ap75, _, _,  _) in enumerate(results_per_category):
                            if ap50 != 'nan':
                                if ind in self.base_inds:
                                    base_ap50_list.append(float(ap50))
                                elif ind in self.novel_inds:
                                    novel_ap50_list.append(float(ap50))
                        logger.info(f"AP_base={np.mean(base_ap50_list)}, AP_novel={np.mean(novel_ap50_list)}, AP_all={np.mean(base_ap50_list + novel_ap50_list)}")
