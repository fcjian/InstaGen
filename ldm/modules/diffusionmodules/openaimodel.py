...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
        ...
        if (timesteps[0] == 1) and (len(all_feature_dic) == 4):
            flag_time = True
        else:
            flag_time = False
        ...
        if flag_time:
            for module in self.input_blocks:
                ...
                reshape_h = h.reshape(2, int(h.size()[0]/2), h.size()[1], h.size()[2], h.size()[3]).permute(1, 0, 2, 3, 4) \
                    .reshape(int(h.size()[0]/2),int(h.size()[1]*2),h.size()[2],h.size()[3])
